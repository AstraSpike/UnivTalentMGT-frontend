      ...
      <div v-if="currentTab === 'create-demand'" class="tab-pane">
        <div class="card">
          ...
          <div class="form-container">
            ...
            <div class="form-item">
              <label>优先级:</label>
              <select v-model="demandForm.taskPriority">
            ...
            <div class="form-group">
              <label class="form-label">接收人员</label>
              <select class="form-control" v-model="demandForm.receiverId">
      ...
      <div v-if="currentTab === 'completed-tasks'" class="tab-pane">
        <div class="card">
          ...
          <div class="filter-bar">
            <div class="filter-item">
              <label>发起人员:</label>
              <select v-model="completedFilter.handler">
                <option value="发起人员">全部人员</option>
                <option value="张院长">张院长</option>
                <option value="李人事">李人事</option>
...
const completedFilter = reactive({
  status: 'all',
  handler: '发起人员', // 将默认值从'all'改为'发起人员'
  dateRange: []
});
...
const submitDemand = async () => {
  try {
    const userInfo = JSON.parse(localStorage.getItem('user-info'));
    const response = await axios.post('/demand/submit', { 
      ...demandForm,
    ...
    if (response.data.code === 200) {
      alert('需求提交成功');
      resetDemandForm();
      activeTab.value = 'myDemands';
    } else {
      alert('需求提交失败: ' + response.data.msg);
    }
  } catch (error) {
...
const filteredCompletedTasks = computed(() => {
  return completedTasks.value.filter(task => {
    if (completedFilter.handler !== '全部' && task.handler !== completedFilter.handler) {
      return false;
    }
